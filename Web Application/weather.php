<?php 
			...
			else
			{
				...
				$locationN = preg_replace('/\s+/','_',$location);
			}
			...
			if($type == "zip")
			{
				...
				if(!preg_match('/^[1-9]\d{4}$/',$location))
					{
			...
			if($xml === false)
			{
					echo "<p>Invalid XML. Fix query and try </p>";
					return NULL;
			}
			$cou=(int)$xml->count();
			$vcount=0;
			...
			for($i=0;$i<$cou;$i++)
			{
				...
				else
				{
					...
					$weatherXML->addChild("feed", urlencode($surl));
					$weatherXML->addChild("link", urlencode($detailsLink));
					...
					for($k =1;$k<6;$k++)
					{
						...
						$attr1=$yweather->forecast->attributes()->day;
						$attr2=$yweather->forecast->attributes()->low;
						$attr3=$yweather->forecast->attributes()->high;
						$attr4=$yweather->forecast->attributes()->text;
